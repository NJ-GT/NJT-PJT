# -*- coding: utf-8 -*-
"""
서울 10개 자치구 각각에 대해 6분면 숙박업 인허가 트렌드 PNG 생성.

목적:
    - 자치구마다 한 장씩(외국인민박/일반숙박/관광숙박 트렌드, 동별 히트맵, top3 동, 2020 vs 2025 비교)
      6분면 종합 패널을 자동 생성 → 발표/보고서에 곧바로 사용.
...
출력:
    - data/license_trend_10gu/{구}_license_trend.png  (10개)

처리 흐름:
    1) 3종 인허가 CSV 일괄 로드 후 자치구 추출
    2) 각 구별로 figure 생성 → 6개 서브플롯 그리기
    3) 데이터 없는 패널은 '데이터 없음' 안내 텍스트로 대체 (안전 처리)
"""
import pandas as pd
import numpy as np
import sys
...
def load_gu(path):
    """
    인허가 CSV 로드 + 인허가연도/구 컬럼 추가 + 10개 자치구 필터링.

    인자:
        path (str): 원본 CSV 경로
    반환:
        DataFrame: '인허가연도' / '구' 가 추가된 GU_10 한정 사본
    """
...
for GU in GU_10:
    ...
    민박_영업 = (
        f1[f1["인허가연도"].between(2020, 2025) & (f1["영업상태명"] == "영업/정상")]
        .groupby("인허가연도")
        .size()
        .reindex(YEARS, fill_value=0)
    )

    # 외국인민박 동×연도 피벗 — 합계 기준 상위 8개 동 추림 (없으면 전체)
    동_pivot_raw = (
        f1[f1["인허가연도"].between(2020, 2025)]
        .groupby(["동", "인허가연도"])
        .size()
    ...
    # 핵심 top3 동 — 선그래프용 (부족하면 전체 동)
    top3_dong = (
        list(동_pivot_raw.sum(axis=1).nlargest(3).index)
        if len(동_pivot_raw) >= 3
        else list(동_pivot_raw.index)
    )

    # 일반숙박업 / 관광숙박업 — 연도×업태 피벗
    숙박_yr = (
    ...
    ax3.set_title("상위 3개 동 연도별 추이", fontsize=11, fontweight="bold", pad=8)
    ...
    if not 숙박_yr.empty and 숙박_yr.sum().sum() > 0:
        ...
        for i, col in enumerate(cols):
            ...
            ax4.bar(
                ...
                vals,
                bottom=bottom,
                color=PALETTE[i % len(PALETTE)],  # 8색을 순환
                alpha=0.85,
                width=0.55,
                label=col,
                zorder=3,
    ...
    # [5] 관광숙박업 업태별 누적 막대 (색상 시작 인덱스를 다르게)
    ...
    if not 관광_yr.empty and 관광_yr.sum().sum() > 0:
        ...
        for i, col in enumerate(cols2):
            ...
            ax5.bar(
                ...
                vals,
                bottom=bottom2,
                color=PALETTE[(i + 3) % len(PALETTE)],  # [4]와 색 겹치지 않게 +3
                alpha=0.85,
                width=0.55,
                label=col,
                zorder=3,
    ...
    ax5.set_title(
        "관광숙박업 업태별 신규 인허가", fontsize=11, fontweight="bold", pad=8
    )
    ...
    # [6] 2020 vs 2025 업종별 비교
    ax6 = fig.add_subplot(gs[1, 2])
    ax6.set_facecolor("white")
    # 3개 업종 (2020값, 2025값) 튜플
    summary = {
        "외국인관광\n도시민박업": (
            int(민박_yr.get(2020, 0)),
            int(민박_yr.get(2025, 0)),
        ),
        "숙박업\n(합계)": (
            int(숙박_yr.sum(axis=1).get(2020, 0)),
            int(숙박_yr.sum(axis=1).get(2025, 0)),
        ),
        "관광숙박업\n(합계)": (
            int(관광_yr.sum(axis=1).get(2020, 0)),
            int(관광_yr.sum(axis=1).get(2025, 0)),
        ),
    }
    labels_s = list(summary.keys())
    v2020 = [summary[k][0] for k in labels_s]
    v2024 = [summary[k][1] for k in labels_s]  # 변수명은 v2024 지만 2025년 값
    x = np.arange(len(labels_s))
    w = 0.32
    # 2020(회색) vs 2025(메인 파랑)
    ax6.bar(
        x - w / 2, v2020, width=w, color="#94A3B8", alpha=0.85, label="2020년", zorder=3
    )
    ax6.bar(
        x + w / 2, v2024, width=w, color=C_MAIN, alpha=0.85, label="2025년", zorder=3
    )
    # 막대 위 숫자 — 0이면 표기 생략
    ...
    ax6.set_xticks(x)
    ax6.set_xticklabels(labels_s, fontsize=9)
