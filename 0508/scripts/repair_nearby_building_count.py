# -*- coding: utf-8 -*-
"""
숙박시설별 '주변건물수/집중도' 결측·0 값 보정 스크립트.

목적:
    - 분석 마스터의 주변건물수가 0 또는 NaN 인 의심 행을 자동 보정해
      이후 회귀/클러스터링의 안정성 확보.

보정 우선순위 (위에서 아래로 시도):
    1) 후보 파일에서 (이름, 위도, 경도) 정확 매칭
    2) (위도, 경도) 만 매칭 (이름 다른 동일 위치)
    3) 이름만 매칭
    4) 30m 이내 좌표 최근접 매칭 (BallTree haversine)
    5) 건물 SHP 로 직접 50m 버퍼 카운트 재산출
    6) 통계 추정 — 구+동+업종 / 구+동 / 구+업종 / 업종 / 전체 중앙값

입력:
    - 0424/분석/tables/분석변수_최종테이블0423_AHP3등급비교.csv (보정 대상)
    - data/clustering_result_all.csv (보정 대상)
    - data/final_spatial_pipeline/analysis_dataset.csv (보정 대상)
    - 프로젝트 내 모든 *.csv/*.xlsx (후보 자동 수집)
    - data/AL_D010_11_20260409/AL_D010_11_20260409_filtered.shp (직접 산출용)

출력:
    - data/final_spatial_pipeline/nearby_building_count_source_inventory.csv
    - data/final_spatial_pipeline/nearby_building_count_all_candidates.csv
    - data/final_spatial_pipeline/nearby_building_count_repair_summary.json
    - {대상파일}_주변건물수보정.csv (각 입력별)
"""
...
# 보정 대상 3개 파일 — 각각 별도 *_주변건물수보정.csv 로 저장
TARGET_FILES = [
...
# 직접 50m 산출용 건물 SHP (도엽 단위 필터링본)
BUILDING_SHP = (
    BASE / "data" / "AL_D010_11_20260409" / "AL_D010_11_20260409_filtered.shp"
)

# 컬럼 후보들 — 데이터마다 컬럼명이 다를 수 있어 여러 후보 동시 인식
BUILDING_COUNT_COLS = [
    "주변건물수_50m",
    "반경_50m_건물수",
    "주변건물수",
...
def read_table(path: Path, nrows: int | None = None) -> pd.DataFrame | None:
    """CSV/Excel 안전 로드 — 실패 시 None 반환 (전체 탐색에서 에러 회피)."""
    try:
...
def inventory_sources() -> pd.DataFrame:
    """프로젝트 내 모든 CSV/Excel 을 헤더만 읽어 '주변건물수 컬럼이 있는' 파일 인벤토리 작성."""
    rows = []
...
def candidate_priority(path: Path, count_col: str) -> int:
    """파일+컬럼 조합에 우선순위 점수 부여 — 신뢰도 높은 출처일수록 큰 값."""
    p = str(path).replace("\\", "/")
    # 사용승인일 유효성 확인된 50m 가 최고 신뢰도
...
def collect_candidates() -> pd.DataFrame:
    """프로젝트 전 파일에서 후보 행을 수집 — (이름, 좌표, 카운트, 집중도, 출처, 우선순위)."""
    frames = []
    for path in BASE.rglob("*"):
        if path.suffix.lower() not in {".csv", ".xlsx", ".xls"}:
            continue
        ...
        # 카운트가 있고, 이름 또는 좌표 중 하나라도 있어야 매칭 가능
    ...
    # 이름 키 + 우선순위 + 카운트 큰 순으로 정렬 (후속 best 추출 안정성)
...
def best_candidates(
    ...
    """후보에서 3종 best 추출 — 이름+좌표 / 좌표만 / 이름만 (각각 우선순위 1순위 채택)."""
    # 0 또는 음수 카운트는 신뢰 불가 — 제외
    candidates = candidates[
        pd.to_numeric(candidates["후보_주변건물수"], errors="coerce").gt(0)
    ].copy()
    # 1) 이름 + 좌표 모두 있는 후보의 best
    coord = (
...
def use_group(code: object) -> str:
    """건축물대장 A8(용도 5자리 코드) → 용도 그룹 라벨 변환."""
...
def direct_building_counts(df: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
    """SHP 의 건물 중심점을 기준으로 50m 버퍼 안 건물 수 + 최빈 용도 비율(=집중도) 직접 산출."""
    result = pd.DataFrame(
...
def repair_target(
    ...
    """단일 대상 파일에 대해 6단계 보정 실행 + *_주변건물수보정.csv 저장."""
    ...
    # ── 4단계: 좌표 30m 이내 최근접 매칭 (BallTree haversine) ──
    ...
    if needs_nearest.any() and len(nearest_source):
        # 라디안 변환 + haversine 거리 트리 — 1 라디안 ≈ 6371000m
    ...
    # ── 6단계: 통계 추정 (구+동+업종 → 구+동 → 구+업종 → 업종 → 전체 중앙값) ──
    ...
    # ── 추가: 집중도 0 행만 단독 보정 (주변건물수는 정상이지만 집중도가 0) ──
    if "집중도" in repaired.columns:
...
def main() -> None:
    """메인 — 인벤토리 → 후보 수집 → best 추출 → 대상 파일별 보정 → 요약 JSON."""
