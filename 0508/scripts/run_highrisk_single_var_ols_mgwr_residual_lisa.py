# -*- coding: utf-8 -*-
"""
고위험군 한정 단일변수 OLS vs MGWR 잔차 LISA 비교 지도 생성 스크립트.

사용 변수 (각 변수마다 별도 PNG 생성):
    구조노후도, 단속위험도, 최근접_소화용수_거리등급, 도로폭위험도

처리 절차 (변수당 1회):
    1) 데이터를 고위험군(cluster_label='고위험군')만 필터링
    2) 단일 변수로 OLS 적합 → 행 단위 잔차
    3) 동일 데이터로 단일 변수 MGWR 적합 → 행 단위 잔차
    4) 잔차를 법정동 평균으로 집계
    5) Queen 인접 가중치로 모란/지역모란 → LISA 분류
    6) OLS/MGWR 잔차 LISA 지도를 한 PNG 로 저장

목적:
    - 고위험군 안에서 각 단일 변수가 설명하지 못한 잔차의 공간 패턴을 진단.
    - OLS(전역) vs MGWR(국지)의 잔차 자기상관 차이를 통해 변수 효과의 공간 이질성 평가.
"""
...
# LISA 카테고리 색상/라벨 — '자료 없음' 라벨이 '고위험군 자료 없음'으로 다름
LISA_COLORS = {
    "HH": "#d7191c",
    "LL": "#2c7bb6",
    "HL": "#fdae61",
...
def load_frame() -> pd.DataFrame:
    """마스터 CSV 에서 고위험군 행만 추출 — cluster_label 우선, 없으면 cluster 최댓값."""
    ...
    # 1차: cluster_label == '고위험군'
    high = work[work["cluster_label"].eq("고위험군")].copy()
    # 2차 fallback: 라벨이 비었을 때 최대 cluster id 를 고위험군으로 가정
    if high.empty:
...
def load_boundary() -> gpd.GeoDataFrame:
    """법정동 경계 GDF 로드 — 자치구/동명 매칭, 5179 좌표계, 무효 지오메트리 보정."""
...
def as_bandwidth_list(raw_bw, expected_len: int) -> list[float]:
    """Sel_BW.bw 의 다양한 형태를 list[float] 로 변환 — 길이 다르면 NaN 패딩."""
...
def run_mgwr(
    ...
    """단일 변수 MGWR 적합 — 절편 + 변수 1개로 BW 탐색 후 적합."""
    ...
    # multi=True 로 변수마다 다른 BW 허용 (절편 + 1변수 = 2개 BW)
    t0 = time.time()
    selector = Sel_BW(
...
def lisa_category(p: np.ndarray, q: np.ndarray) -> np.ndarray:
    """LISA quadrant + p값 → HH/LH/LL/HL/Not Sig 라벨."""
    sig = p < 0.05
...
def residual_lisa(
    ...
    """모델별 잔차를 동 평균으로 집계 → 모란/지역모란 → 결과 GDF + 지표."""
    # 동 단위 평균 잔차 + 표본 수
    dong = (
    ...
    # base 에 LISA 컬럼 재결합 — '자료 없음' 까지 포함
    out = base.merge(
...
def plot_lisa(
    ...
    """변수 1개에 대해 OLS/MGWR LISA 지도를 좌우로 그려 PNG 저장."""
...
def main() -> None:
    """변수 4개 각각 OLS/MGWR/LISA 실행 후 비교지도와 메타 일괄 저장."""
