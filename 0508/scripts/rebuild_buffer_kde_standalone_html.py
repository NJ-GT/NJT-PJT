# -*- coding: utf-8 -*-
"""
버퍼(50m) + KDE 건물 밀집도 시각화를 PNG + 단독 HTML 로 재생성.

목적:
    - 외부 CDN 없이 오프라인에서도 열리는 단독 HTML 산출물 생성.
    - 50m 버퍼 내 주변건물수를 가중치로 한 KDE 등고선 + 분류 산점도 + 자치구 경계 + 상위 12개 시설 표.

입력:
    - data/서울10구_숙소_소방거리_유클리드.csv         (위도/경도/반경_50m_건물수/구/동/업소명/집중도(%))
    - data/seoul_legal_dong_age_buckets_joined_0415.geojson  (법정동 경계)

출력:
    - data/Map_Buffer_KDE_standalone.png   (matplotlib 이미지)
    - data/Map_Buffer_KDE_standalone.html  (이미지를 base64 로 임베드한 단독 HTML)
"""
from __future__ import annotations

import base64  # PNG → base64 인코딩 (HTML 임베드용)
import json
...
import numpy as np
import pandas as pd
from scipy.stats import gaussian_kde  # 2D 가우시안 KDE
from shapely.geometry import shape  # GeoJSON → shapely
from shapely.ops import unary_union  # 자치구별 폴리곤 합치기
...
def set_korean_font() -> None:
    """matplotlib 한글 폰트 — 맑은 고딕."""
    plt.rcParams["font.family"] = "Malgun Gothic"
    plt.rcParams["axes.unicode_minus"] = False


def iter_polygon_rings(geometry: dict):
    """GeoJSON geometry dict → 외곽선/구멍 ring 좌표 리스트 제너레이터."""
    ...
    if gtype == "Polygon":
        ...
        # MultiPolygon: [[polygon1_rings], [polygon2_rings], ...]
...
def iter_shapely_rings(geom):
    """shapely 지오메트리에서 외곽선 좌표 리스트만 추출 (구멍 무시)."""
    if geom.is_empty:
        return
...
def add_gu_boundaries(ax, path: Path) -> None:
    """자치구 단위로 폴리곤을 합쳐 굵은 외곽선 + 구 라벨 그리기."""
    if not path.exists():
        return
    with path.open(encoding="utf-8") as f:
        geo = json.load(f)
...
def build_kde_image() -> tuple[str, pd.DataFrame]:
    """KDE 이미지를 그려 (base64 문자열, 사용한 DataFrame) 을 반환."""
    set_korean_font()
    ...
    # 서울 범위 (대략) 밖 좌표는 제거 — 이상치 방지
    df = df[
        (df["위도"].between(37.4, 37.7)) & (df["경도"].between(126.7, 127.3))
    ].copy()

    # KDE 가중치 — 반경 내 건물수 (합이 0이면 균등 가중치로 fallback)
    ...
    # bw_method=0.04 — 비교적 좁은 대역폭 (디테일 강조)
    ...
    # 'YlOrRd' 컬러맵에 알파를 직접 가공 — 낮은 값은 투명, 중간부터 점진적 불투명
    cmap = plt.get_cmap("YlOrRd")
    cmap_alpha = cmap(np.linspace(0, 1, 256))
    cmap_alpha[:28, 3] = 0  # 0~28 구간 완전 투명
    cmap_alpha[28:58, 3] = np.linspace(0.05, 0.45, 30)  # 28~58 점진 불투명
    custom_cmap = mcolors.LinearSegmentedColormap.from_list("YlOrRd_alpha", cmap_alpha)

    # ── Figure / 배경/등고선/경계/포인트 ──
    ...
    # 점 분류 — 건물수 30 초과(빨강) / 15~30(주황) / 15 미만(초록)
    bins = [
    ...
    # KDE 그라디언트 컬러바 — 직접 axes 추가해 작은 띠로 표시
    ...
    # 동일 figure 를 메모리 버퍼로도 출력 → base64 로 인코딩
    buf = BytesIO()
...
def write_html(image_b64: str, df: pd.DataFrame) -> None:
    """이미지를 base64 로 임베드한 단독 HTML 작성 — 외부 의존 없음."""
    # 건물 밀집 상위 12개 시설 표
    top_rows = (
...
def main() -> None:
    """KDE 이미지 생성 → 단독 HTML 작성 → 결과 경로 출력."""
