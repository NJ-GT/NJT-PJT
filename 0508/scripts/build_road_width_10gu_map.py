# -*- coding: utf-8 -*-
"""
서울 10개구 도로폭 대표좌표를 인터랙티브 Leaflet 지도(HTML)로 시각화하는 스크립트.

목적:
    - 도로 단위 대표 좌표(서울시 도로 viRoutDt 기반)를
      구/도로폭 필터 + 검색 + 통계 패널이 있는 단독 HTML 로 출력 → 외부 의존 최소.

입력:
    - road_width_10gu/data/seoul_road_width_viRoutDt_10개구_대표좌표.csv

출력:
    - road_width_10gu/data/seoul_road_width_10gu_map.html  (Leaflet 지도)

특징:
    - 도로폭 12분류별 색상/반지름 매핑 (좁은 도로일수록 녹색·작게, 넓은 도로일수록 어두운 색·크게)
    - 좌측 패널: 검색 입력 + 구/도로폭 체크박스 필터 + 구별 막대 + 도로폭 색상 범례
"""
...
# 경로 (별도 road_width_10gu/ 모듈 폴더 사용)
...
# 도로폭별 색상 — 녹색(좁음/안전 위주) → 빨강·보라 → 짙은 파랑·검정(넓은 도로)
WIDTH_COLORS = {
    "6m미만": "#2f855a",
    "폭6-8m": "#63a15f",
    "폭8-10m": "#d6a419",
...
def count_by(
    ...
    """key 값별 점 카운트 → 사전 정의 순서로 정렬 + 0건 제외."""
...
def build_html(points: list[dict[str, object]]) -> str:
    """단독 HTML 문자열 생성 — Leaflet + 인라인 CSS + JS 모두 포함."""
    # 패널 통계 — 구별/도로폭별 카운트
    ...
    # JS 임베드용 JSON 문자열 — 한글 깨짐 방지 + 줄바꿈 제거(파일 크기)
...
<body>
  ...
  <script>
    // 파이썬에서 임베드한 데이터 — 페이지 로드 직후 즉시 사용
