# -*- coding: utf-8 -*-
"""
팀이 만든 위험도 점수 + 모델링 파이프라인을 검증하는 일괄 점검 스크립트.

목적:
    - 팀의 '위험도점수' 산출식과 GWR/Lasso/RF/OLS 결과가
      실제 화재 데이터(2021~2024)에 대해 얼마나 설명력을 갖는지 한 번에 평가.
    - 결과를 모두 CSV/JSON/PNG 로 저장해 보고서/발표 근거로 사용.

검증 단계:
    01_clustering    : KMeans (k=4) + Elbow + 군집별 화재 통계
    02_lasso         : 화재가 발생한 시설만 학습해 위험요인 변수 선택
    03_team_risk     : 팀의 위험도점수 산식 적용 → 상위 30 시설 추출
    04_moran         : 위험도점수 공간자기상관 (위경도 vs EPSG:5181)
    05_gwr           : 표본(<=650) GWR 적합 + 국지 계수 저장
    06_rf            : 순환검증(A/B) + 진짜검증(C: 화재수, D: 피해액)
    07_ols_sanity    : 위험도점수/실제 화재수/피해액에 대한 OLS 변수별 효과

입력:
    - 0424/분석/tables/분석변수_최종테이블0423_AHP3등급비교_주변건물수보정.csv
    - data/화재출동/화재출동_2021_2024.csv

출력:
    - data/team_pipeline_validation/  (CSV + PNG + validation_summary.json)
"""
...
import statsmodels.api as sm  # OLS 점검용
# 공간통계 + 모델
...
# GWR 설명변수 — 위험도점수 영향 분석용 4변수
GWR_X_VARS = ["구조노후도", "도로폭위험도", "주변건물수", "집중도"]
RANDOM_STATE = 42  # 모든 무작위 과정의 시드 통일


def read_csv(path: Path) -> pd.DataFrame:
    """UTF-8 BOM 지원 + low_memory=False 로 안전 로드."""
    return pd.read_csv(path, encoding="utf-8-sig", low_memory=False)


def load_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    """분석 마스터와 화재 데이터를 로드 + 결측/형변환 정리."""
    acc = read_csv(ACC_PATH).copy()
    fire = read_csv(FIRE_PATH).copy()
    # 발생연도 숫자형 + 2021~2024 필터
...
def add_fire_targets(
    ...
    """
    각 시설 좌표 기준 반경 radius_m 내에서 발생한 화재 통계를 컬럼으로 부착.

    BallTree(haversine) 으로 위경도 거리 검색 — 반경(m)을 지구반지름으로 나눠 라디안.
    """
    result = acc.copy()
    # 위경도를 라디안으로 — haversine 거리 함수의 입력
    ...
    # query_radius — 시설마다 반경 내 화재 인덱스 리스트
...
def run_clustering(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """KMeans Elbow(k=1~10) + 최적 k=4 라벨 부여 + 군집별 화재 통계 요약."""
    # 표준화 후 인풋 — 결측은 0으로 채움
    x_scaled = StandardScaler().fit_transform(df[FEATURES].fillna(0))
    inertia_rows = []
    # k 1~10 까지 inertia(SSE) 측정
    for k in range(1, 11):
...
def run_lasso(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    """
    화재 발생 시설만 모아 log(피해액)을 Lasso 로 회귀 — 변수 선택 효과 확인.

    alpha=0.01 고정과 LassoCV(자동 alpha) 두 결과를 함께 비교.
    """
...
def add_team_risk_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    설계:
        - 5개 위험변수를 MinMax 정규화
        - 주변건물수/집중도는 '낮을수록 고립/사각지대' 의미라 1- 로 반전
        - 가중치: 고립 0.35 + 사각지대 0.20 + 도로폭 0.15 + 노후 0.15 + 단속 0.15
        - ×100 → 0~100 점수
    """
    result = df.copy()
    ...
    # 위험도 상위 30개 시설만 별도 저장 (검증용 시각 점검)
...
def run_moran(df: pd.DataFrame) -> pd.DataFrame:
    """위험도점수에 대해 모란 I — 위경도 그대로와 EPSG:5181 평면좌표 두 버전 비교."""
    rows = []
    # 1) 위경도 그대로 — 위도/경도가 거리 단위로 다름(일그러짐)
...
def run_gwr_sample(df: pd.DataFrame, max_rows: int = 650) -> tuple[pd.DataFrame, dict]:
    """샘플(<=650)에 대해 GWR 적합 — 시간 단축을 위해 샘플링 + 국지 계수 저장."""
    work = df.dropna(subset=["x_5181", "y_5181", "위험도점수"] + GWR_X_VARS).copy()
    # 표본이 너무 크면 무작위 샘플링 (인덱스 정렬해 결과 재현 가능)
...
def run_rf_checks(
    ...
    """
    RF 회귀로 4가지 검증을 일괄 수행.
        A: 위험도점수 → 위험도점수 자체를 구성변수로 재예측 (순환검증)
        B: 위험도점수 → A + GWR 국지계수도 함께 사용 (정보 누설성)
        C: 실제 화재수(log1p) 예측 — 진짜 검증
        D: 실제 피해액(log1p) 예측 — 진짜 검증
    """
    rows = []

    def fit_rf(name: str, data: pd.DataFrame, features: list[str], target: str) -> None:
        """단일 RF 적합 후 행 dict 누적."""
...
def run_ols_sanity(df: pd.DataFrame) -> pd.DataFrame:
    """
    3개 종속변수(위험도점수/log1p 화재수/log1p 피해액)에 대해 OLS 적합.

    HC3 강건 표준오차 — 이분산성에 견고. 변수별 계수와 p-value 를 한 표로 정리.
    """
    rows = []
    for target in ["위험도점수", "log1p_fire_count_150m", "log1p_damage_sum_천원"]:
        work = df.copy()
        # log1p 변환 종속변수는 즉석에서 계산
...
def main() -> None:
    """전체 검증 파이프라인 실행 + 통합 요약 JSON 저장."""
    acc, fire = load_data()
    # 화재 타깃 부착 → 클러스터링 → Lasso → 위험도점수 → 모란 → GWR → RF → OLS
