# -*- coding: utf-8 -*-
"""
3변수 OLS + 군집별 MGWR 적합 후, 두 모델의 잔차 LISA 지도를 한 PNG로 비교.

목적:
    - 핵심 3개 위험변수(구조노후도/도로폭위험도/단속위험도)만 사용한 단순 모델에서
      OLS 와 MGWR 의 잔차 공간 군집 패턴이 어떻게 다른지 진단.
    - MGWR 은 cluster (저/중/고위험군) 단위로 분리 적합 후 결과를 합쳐 잔차 지도를 그림.

입력:
    - 0430/*0429.csv                                        : 분석 마스터 테이블(0429 일자)
...
출력:
    ...
    - data/three_var_residual_lisa/{ols|mgwr}_3var_residual_lisa_by_dong.csv
    - data/three_var_residual_lisa/residual_lisa_summary.csv
    - data/three_var_residual_lisa/run_metadata.json
    - data/three_var_residual_lisa/ols_mgwr_3var_residual_lisa_maps.png
"""

from __future__ import annotations

import json  # 메타데이터 직렬화
import time  # MGWR 시간 측정
...
# 평면 좌표 (EPSG:5181 — 중부원점) — MGWR 거리 계산용
COORDS = ["x_5181", "y_5181"]
# 결과 CSV 에 함께 보존할 식별/메타 컬럼
ID_COLS = [
    "구",
    "동",
    "숙소명",
...
def standardize_xy(df: pd.DataFrame):
    """MGWR 입력 형식으로 변환 — 좌표는 그대로, X는 표준화, y는 (n,1) 모양."""
...
def run_ols(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    """전체 데이터에 대해 단일 OLS 적합 — 잔차/계수를 CSV로 저장."""
    x = StandardScaler().fit_transform(df[FEATURES].astype(float).to_numpy())
...
def as_bandwidth_list(raw_bw, expected_len: int) -> list[float]:
    """
    Sel_BW.bw 의 다양한 반환 형태(tuple/array)를 평탄한 list[float] 로 변환.

    예상 길이와 다르면 NaN 패딩을 반환 — 후속 컬럼 작성에서 안전하게 처리.
    """
...
def fit_mgwr_group(
    ...
    """단일 cluster 그룹에 대해 MGWR 을 적합하고 잔차/계수를 반환."""
    coords, y, x = standardize_xy(group)
    # 군집 라벨 — 그룹의 첫 비결측 값 / 없으면 ID 문자열
    label = (
    ...
    # 변수마다 다른 bandwidth 탐색 (multi=True)
    t0 = time.time()
    selector = Sel_BW(
    ...
    # 본 MGWR 적합 — 위 selector 의 bandwidth 결과 사용
    t0 = time.time()
    result = MGWR(
    ...
    # intercept + 3 변수 — 각각 계수와 bandwidth 기록
...
def run_mgwr(df: pd.DataFrame, n_jobs: int = 1) -> tuple[pd.DataFrame, list[dict]]:
    """cluster (저/중/고) 별로 MGWR 을 분리 적합한 뒤 결과를 합쳐 한 CSV로 저장."""
    outputs = []
    metrics = []
...
def load_boundary() -> gpd.GeoDataFrame:
    """법정동 경계 GDF 로드 — 자치구/동명 매칭 컬럼 부착, 5179 좌표계, 무효 지오메트리 보정."""
...
def lisa_category(p: np.ndarray, q: np.ndarray) -> np.ndarray:
    """LISA 분면(q)+p값 → 카테고리 라벨로 변환 — p ≥ 0.05 면 'Not Sig'."""
    sig = p < 0.05
...
def residual_lisa(
    ...
    """모델 잔차를 동 평균으로 집계 → Queen 가중치 → LISA 분류 → CSV/지도 입력 GDF 반환."""
    # (구, 동) 단위 평균 잔차와 표본 수
    dong = (
    ...
    # 원본 GDF 에 LISA 컬럼을 다시 합쳐 — '자료 없음' 동까지 모두 포함된 결과
    out = gdf.merge(
...
def main() -> None:
    """전체 파이프라인 실행: 데이터 → OLS → 군집별 MGWR → LISA → 지도/메타 저장."""
    ...
    # 2) cluster 단위 MGWR (n_jobs=1 — 안정성 우선)
