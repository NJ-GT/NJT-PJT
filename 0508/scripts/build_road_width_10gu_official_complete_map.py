# -*- coding: utf-8 -*-
"""
서울 10개구 도로폭을 행안부 공식 도로구간 SHP(TL_SPRD_MANAGE)와 합쳐 완성형 지도 생성.

기존 build_road_width_10gu_line_map.py 와 차이:
    - 도로 선형의 출처가 OSM 이 아닌 행안부 공식 도로 SHP (RDS_MAN_NO 단위)
    - 도로폭 결정 우선순위: API 도로폭 → 공식 ROAD_BT 폭 fallback → 공식만 추가
    - '폭출처' 컬럼으로 어디서 폭을 가져왔는지 명시 (API / 공식폭보완 / 공식만)
    - 공식만(API 매칭 안 된 도로) 은 지도에서 dashed 라인으로 시각적 구분

입력:
    - road_width_10gu/data/seoul_road_width_viRoutDt_10*.csv      (API 도로폭)
    - road_width_10gu/data/official_road_shape_202603_seoul/11000/TL_SPRD_MANAGE.shp

출력:
    - road_width_10gu/data/seoul_road_width_10gu_official_complete_map.html
    - road_width_10gu/data/seoul_road_width_10gu_official_complete_lines.geojson
    - road_width_10gu/data/seoul_road_width_10gu_official_complete_api_unmatched.csv
    - road_width_10gu/data/seoul_road_width_10gu_official_only_roads.csv
"""
...
# 행안부 SHP 의 좌표계 (UTM-K) — 위경도(4326) 로 변환 후 사용
SRC_CRS = "EPSG:5179"
...
def official_width_bucket(value: float | None) -> str:
    """공식 ROAD_BT(미터) → API 와 동일한 카테고리 라벨 변환."""
...
def distance_m(lat1: float, lng1: float, lat2: float, lng2: float) -> float:
    """위경도 두 점 사이 거리(m) — 위도 평균 cosine 보정 평면 근사."""
...
def perpendicular_distance_m(
    ...
    """점에서 (start, end) 직선까지 수직 거리(m) — Douglas-Peucker 입력."""
...
def simplify_line(
    ...
    """Douglas-Peucker — 공식 SHP 는 OSM 보다 정밀해 tolerance 더 작게(1.2m)."""
    if len(coords) <= 2:
        return coords
    start = coords[0]
    end = coords[-1]
...
def geometry_to_lines(geom: LineString | MultiLineString) -> list[list[list[float]]]:
    """Shapely 라인 → [[x, y], ...] 리스트 (GeoJSON 좌표 순서)."""
    if geom.is_empty:
        return []
...
def choose_endpoints(lines: list[list[list[float]]]) -> tuple[list[float], list[float]]:
    """모든 라인의 끝점 중 가장 멀리 떨어진 두 점을 시작/끝으로."""
    endpoints = []
    for line in lines:
        if len(line) >= 2:
...
def load_input() -> pd.DataFrame:
    """API 도로폭 CSV — 첫 7개 컬럼을 표준 이름으로 매핑 후 (구, 도로명) 키 부착."""
    df = pd.read_csv(INPUT_CSV, encoding="utf-8-sig", dtype=str).fillna("")
    # 입력의 첫 7개 컬럼이 의미상 (순번, 구, 도로명, 구분, 기능, 규모, 폭)
...
def load_official() -> gpd.GeoDataFrame:
    """행안부 공식 도로 SHP 로드 — cp949 인코딩, EPSG:5179 → 4326 변환."""
    gdf = gpd.read_file(OFFICIAL_SHP, engine="pyogrio", encoding="cp949")
    # SIG_CD 5자리 → 자치구명, 매칭 안되면 빈 문자열로 (서울 10구 외 행 제거)
...
def build_features(
    ...
    for key, group in official.groupby("_key", sort=False):
        ...
        if key in meta_by_key:
            ...
            # 공식에만 있는 도로 — 공식 폭 그대로 사용 + 메타데이터 누적
            first = group.iloc[0]
            api_width = ""
            display_width = fallback_width
            width_source = "공식만"
        ...
        props = {
            ...
            # 한 도로에 여러 RN_CD 가 있을 수 있어 정렬+조인
            "RN_CD": "|".join(
    ...
    # 빈 DataFrame 도 같은 컬럼 스키마로 — 후속 to_csv 안전
...
def source_counts(features: list[dict]) -> dict[str, int]:
    """폭출처별(API/공식폭보완/공식만) 도로 수 — 패널 메타용."""
...
def leaflet_rows(features: list[dict]) -> list[dict[str, object]]:
    """GeoJSON 피처 → Leaflet JS 가 직접 사용할 dict 평탄화."""
...
def build_html(
    ...
    """단독 HTML 생성 — Leaflet + 좌측 검색·필터 패널 + 폭출처 정보 표시."""
...
<body>
  ...
  <script>
    ...
    function showEndpoints(row) {{
      // 클릭 강조 — 라운드 캡으로 공식 SHP 정밀선과 어울리게
    ...
    function render() {{
      ...
      rows.forEach((row) => {{
        row.lines.forEach((line) => {{
          ...
          // 공식만 도로는 dashed 라인으로 시각적 구분 (실선은 API 매칭됨)
...
def main() -> int:
    """메인 — API 로드 → 공식 SHP 로드 → 결합 → GeoJSON/CSV/HTML 저장 + 콘솔 통계."""
    input_df = load_input()
