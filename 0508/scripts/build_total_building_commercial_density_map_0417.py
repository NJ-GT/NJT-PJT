# -*- coding: utf-8 -*-
"""
서울 10개구 건물·상가 밀집도 인터랙티브 Leaflet 지도 + 실제 건물 위치 오버레이 생성.

build_commercial_lodging_density_0417.py 와 차이/확장:
    - 출력의 베이스 격자(50,000㎡) 는 동일 GeoJSON 사용
    - 추가로 '건축면적 × 층수' 보정분자 / 표제부(table) 보정 / 0밀도 제외 처리 포함
    - 격자 위에 '실제 건물 포인트' 레이어를 겹쳐볼 수 있는 JS 데이터 별도 생성

핵심 보정 로직:
    1) 건축면적이 0이면 연면적/총층수 로 역추산 (계산건축면적)
    2) 건축면적×총층수 가 연면적보다 크면 연면적으로 캡 (이상치 보정)
    3) 표제부(table) 후보 CSV 가 있으면 건물 면적/층수/용도/이름을 표제부 값으로 교체
    4) 0밀도 제외 CSV 에 명시된 (그리드ID, 건물ID) 는 보정되지 않은 한 집계에서 제외

입력:
    - data/AL_D010_11_20260409/AL_D010_11_20260409_filtered.shp        (건물 SHP)
    - data/상가숙소밀집도_10개구_0417/서울10개구_상가숙소_격자별_밀집도.geojson (베이스 격자)
    - data/건물상가밀집도_10개구_0417/서울10개구_밀도0_제외건물목록_388.csv (선택)
    - data/건물상가밀집도_10개구_0417/서울10개구_건물있지만_밀도0_표제부보정후보_건물목록.csv (선택)

출력 (모두 data/건물상가밀집도_10개구_0417/):
    - 서울10개구_건물상가_50,000sqm_밀도지도.html        (Leaflet 지도)
    - actual_buildings_10gu_0417.js                        (건물 포인트 JS)
    - 서울10개구_건물상가_격자별_시각화.csv
    - 서울10개구_건물상가_격자별_밀도.geojson
    - 서울10개구_건물상가_시각화컬럼정의.csv
    - 서울10개구_건물상가_산출요약.json
"""
...
def to_float(value: object) -> float:
    """안전 float 변환 — None/빈문자/NaN/inf 는 0.0."""
    try:
...
def quantile(values: list[float], q: float) -> float:
    """양수만 추려 분위수 계산 — 캡 산정용 (p98 등). 빈 리스트면 1.0."""
...
def load_zero_density_exclude_keys() -> set[tuple[str, str]]:
    """0밀도 제외 CSV → (그리드ID, 건물ID) Set — 표제부 보정 안 된 건물만 적용."""
...
def load_registry_corrections() -> pd.DataFrame:
    """표제부 보정 후보 CSV 로드 — 보정분자 > 0 인 행만 keep."""
    if not REGISTRY_CORRECTION_CSV.exists():
        return pd.DataFrame()

    # 코드/PK 는 문자열로 (선행 0 보존)
    dtype = {
        "그리드ID": str,
        "건물ID": str,
...
def apply_registry_corrections(gdf: gpd.GeoDataFrame) -> tuple[gpd.GeoDataFrame, int]:
    """SHP 0면적 건물에 표제부 후보값으로 면적/층수/용도/이름 덮어쓰기."""
    # 보정 컬럼 초기화 — 모든 행 기본값
    gdf["건물명"] = ""
    gdf["관리건축물대장PK"] = ""
    gdf["표제부보정여부"] = False
...
def calculate_building_metrics(gdf: gpd.GeoDataFrame) -> tuple[gpd.GeoDataFrame, int]:
    """건물별 면적/층수 보정분자 산출 + 상가 분류 + 입체화재하중 분자 두 가지 산출.

    분자 정의:
        - 건축면적x층수_분자: 계산건축면적 × 총층수 (역추산 포함, 연면적으로 캡)
        - 전체건물입체화재하중_분자: 건축면적×총층수 vs 연면적 비교 후 더 안전한 값
    """
    # 개별 건물 데이터 가용성 플래그
    ...
    # 건축면적은 없지만 연면적/층수가 있어 역추산 가능한 건물
    gdf["건축면적추정여부"] = (
        (~gdf["건축면적있음"]) & gdf["연면적있음"] & gdf["층수있음"]
    )
    # 계산건축면적 = 원본 우선, 추정 가능하면 연면적/총층수 로 역추산
    ...
    # 건축면적×총층수 가 연면적을 초과하면 연면적으로 캡 (이상치 보정)
    ...
    # 면적 보정 트리거 — 다음 중 하나에 해당하면 연면적으로 대체
    use_floor_area = (gdf["연면적"] > 0) & (
        (gdf["건축면적"] <= 0)
        | (gdf["건축면적"] > gdf["연면적"])  # 건축면적이 연면적보다 큰 이상치
        | (floor_proxy > gdf["연면적"])  # 건축면적×층수가 연면적 초과
    )
    gdf["면적보정여부"] = use_floor_area
    # 보정 전 원본 분자 — 비교용 (감사 추적)
    gdf["전체건물입체화재하중_분자_원본"] = floor_proxy.where(
        floor_proxy > 0, gdf["연면적"]
    )
    # 보정 후 분자 — 이상치는 연면적으로 대체
    gdf["전체건물입체화재하중_분자"] = floor_proxy.where(~use_floor_area, gdf["연면적"])
    # 그래도 0인 행은 연면적으로 fallback
...
def load_base_grid() -> tuple[dict, pd.DataFrame, float, float]:
    """베이스 격자 GeoJSON 로드 + 격자 행/열 ID 로 origin_x/y 추정 (중앙값)."""
    with BASE_GRID_GEOJSON.open("r", encoding="utf-8") as f:
        geojson = json.load(f)

    # 모든 격자의 메타를 평탄한 DataFrame 으로
    rows = []
    ...
    # 격자 origin (좌하단) — 모든 격자에서 동일해야 하므로 중앙값으로 안정 추출
    origin_x = (
...
def build_building_points(gdf: gpd.GeoDataFrame) -> list[list[object]]:
    """실제 건물 포인트 — JS 가 직접 사용할 list[list] 평탄화 (오버레이 레이어용)."""
    ...
    # JS 의 BUILDING_POINT_FIELDS 와 정확히 같은 순서로 18개 컬럼
    points = []
    rows = zip(
        lat_values,
        lng_values,
...
def load_and_aggregate_buildings(
    ...
    # 베이스 격자에 매칭 안 되는 건물 제거 (베이스 격자 외 영역)
...
def build_grid_dataset(
    ...
    """베이스 격자에 건물 집계 결합 + 파생 밀도 컬럼 + GeoJSON/CSV/메타 산출."""
    ...
    # 베이스 GeoJSON 의 geometry 를 보존하고 properties 만 새 데이터로
    features_by_id = {
...
def build_html(geojson: dict, caps: dict, summary: dict) -> str:
    """단독 Leaflet HTML 생성 — 13개 색상 지표 토글 + 실제 건물 오버레이.

    HTML 안에 GeoJSON/CAPS/SUMMARY 를 임베드 (오프라인 동작).
    f-string 안의 CSS/JS 의 { } 는 모두 { { } } 로 이스케이프.
    """
...
// 7단계 파랑 그라디언트 — 낮은 밀도(연한 파랑) → 높은 밀도(짙은 파랑)
const BLUE_SCALE = [
...
// 실제 건물 점은 줌 15 이상에서만 표시 (성능 / 가독성)
const ACTUAL_BUILDING_MIN_ZOOM = 15;
// 한 번에 그리는 점 한도 — 더 많으면 일부만 표시
...
function getProp(feature, field) {{
  // 안전 숫자 추출 — NaN/undefined 는 0
  const value = Number(feature.properties[field] || 0);
  return Number.isFinite(value) ? value : 0;
}}

function getColor(feature) {{
  // p98 캡 기준 0~1 스케일 → BLUE_SCALE 7단계
...
function gridStyle(feature) {{
  // 0 격자는 옅은 회색 테두리, 값 있는 격자는 진한 테두리
  const value = getProp(feature, METRICS[currentMetric].field);
  return {{
...
function labelText(feature) {{
  // 격자 안 라벨 — 0이면 zeroDecimals(보통 0), 그 외는 decimals
...
// 격자 GeoJSON 레이어 — 마우스오버 시 강조 + 클릭 시 팝업
...
function actualBuildingTooltip(item) {{
  // 실제 건물 점 툴팁 — 표제부 보정 여부 / 면적 추정 여부 명시
...
function renderActualBuildings() {{
  // 실제 건물 점 — 줌 15+ + 화면 bbox 안 + 8000개 한도
  ...
  // 화면 영역 8% 패딩 — 경계 가장자리에 점이 갑자기 나타나는 현상 방지
...
function scheduleActualBuildingRender() {{
  // 줌/패닝 직후 디바운스 — 120ms 안에 추가 이동이 있으면 묶어서 한 번만
  if (!actualBuildingsVisible) return;
...
def main() -> None:
    """메인 — 베이스 격자 → 건물 집계+보정 → 격자 결합 → CSV/GeoJSON/JS/HTML 일괄."""
