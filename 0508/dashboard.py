# -*- coding: utf-8 -*-
"""
서울시 관광 지역 내 숙박시설 화재 위험도 분석 대시보드 (Streamlit).

대시보드 구성:
    1) 인허가 추이 — 자치구별 외국인민박/관광숙박/일반숙박업 트렌드
    2) 위험점수/군집 — AHP 기반 시설별 점수 + K=3 군집 시각화
    3) 공간모형 — OLS/SLM/SEM/GWR/MGWR 비교 + 군집별 대표 모형
    4) GWR/MGWR 격자 지도 — 변수별 영향력 공간 이질성
    5) 화재 출동 경로 — 인접 소방서/안전센터/구조대 → 시설 최단경로 + 도로폭 보정 도착시간
    6) 도로폭 시각화 — 행안부 공식/OSM 매칭

핵심 데이터 소스 (모두 BASE 하위 상대경로):
    - 0430/최종테이블0429.csv                 — 시설별 위험점수/군집 마스터
    - 0430/시설별_화재위험_성적표_0429_v3.csv — 카드형 점수 표
    - 0430/가중치군집_*표.csv                  — 공간모형 비교/대표모형 BW
    - data/gwr_results.csv                     — GWR 행 단위 local R²/계수
    - data/mgwr_local_params_low_high.csv     — MGWR low/high BW 변수 계수
    - data/seoul_legal_dong_age_buckets_joined_0415.geojson — 법정동 경계
    - data/grid_spatial_dashboard/seoul_grid_*m.geojson    — 격자 (250/500m)
    - 원본데이터/서울시 *숙박업 인허가 정보.csv         — 인허가 트렌드 원본
    - data/서울10구_숙소_소방거리_유클리드.csv          — 시설↔소방거리 베이스
    - road_width_10gu/data/*                            — 도로폭 SHP/OSM 매칭 결과

캐시 전략:
    - 모든 데이터 로더에 @st.cache_data — 페이지 재실행 시 IO 비용 회피.
...
import json
import math
import heapq    # 다익스트라 우선순위 큐 (소방서→시설 최단경로)
import ast      # 사전 계산된 BW dict 문자열을 파싱
import re
...
# 폴리곤/포인트 지오메트리 — 동/구 매칭, 경계 그리기에 사용
from shapely.geometry import Point, shape
from shapely.ops import unary_union  # 자치구별 폴리곤 dissolve
...
def read_csv(path: Path) -> pd.DataFrame:
    """CSV 안전 로드 — utf-8-sig → cp949 → euc-kr → utf-8 순으로 인코딩 시도."""
...
def load_0430() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """대시보드 핵심 4종 데이터셋 일괄 로드.

    반환:
        final_df  : 최종테이블0429.csv (시설별 위험점수/군집/좌표/변수)
        score_df  : 시설별_화재위험_성적표_0429_v3.csv (카드용 점수)
        model_df  : 가중치군집_대표모형_최종결과표.csv (군집별 BW/AICc)
        compare_df: 가중치군집_공간모형_비교표.csv (모형별 R²/AIC 비교)
    """
...
def load_gwr_local(final_df: pd.DataFrame) -> pd.DataFrame:
    """GWR 행 단위 결과(좌표/local_R²/계수) 로드 + 시설별 메타와 결합."""
...
def load_mgwr_cluster_local() -> pd.DataFrame:
    """MGWR 군집별 적합 결과 로드 — 행 단위 local R²/계수/잔차 + 입력 변수 결합."""
...
def load_dong_boundaries() -> dict:
    """법정동 경계 GeoJSON 로드 — 지도 위 경계선/색칠용."""
...
def load_seoul_grid(cell_size_m: int = 500) -> dict:
    """서울시 격자 GeoJSON 로드 — 250m 또는 500m 셀 (GWR/MGWR 격자 시각화용)."""
...
def parse_bw_table(
    ...
    """군집별 MGWR BW(dict 문자열) 를 변수×군집 표로 펼침 — 변수마다 다른 BW 시각화용."""
    if (
        model_df.empty
...
def load_license_sources() -> pd.DataFrame:
    """3종 인허가 CSV(외국인민박/관광숙박/일반숙박) 일괄 로드 + 합쳐서 반환."""
    sources = [
...
def flatten_road_coordinates(geometry: dict) -> list[list[tuple[float, float]]]:
    """LineString/MultiLineString 지오메트리 → 평탄한 라인 좌표 리스트."""
    if not geometry:
        return []
...
def synthetic_road_line(
    ...
    """매칭 안된 도로 시각화용 — 좌표 1개에서 길이/방위로 가짜 라인 2점 생성."""
    half = length_m / 2
...
def load_unmatched_road_points() -> pd.DataFrame:
    """OSM 매칭 실패 도로의 좌표 포인트 로드 — 지도에 별도 마커로 표시."""
...
def load_road_width_lines() -> pd.DataFrame:
    """공식 도로 선형 GeoJSON 로드 — 도로폭 카테고리/메타 + 위경도 라인 좌표."""
...
def load_road_route_lines() -> pd.DataFrame:
    """OSM 매칭된 도로 선형 GeoJSON 로드 — 출동경로 그래프 빌드의 입력."""
...
def load_route_sources() -> tuple[
    ...
    """출동경로 시각화용 5종 데이터 로드 — 시설/소방/도로폭/도로라인/OSM라인."""
...
def apply_chart_style(fig: go.Figure) -> go.Figure:
    """Plotly figure 에 공통 스타일(폰트/배경/컬러웨이) 적용."""
    fig.update_layout(
        plot_bgcolor="white",
        paper_bgcolor="white",
...
def license_cumulative_chart(license_df: pd.DataFrame, gu: str) -> go.Figure:
    """자치구 연도별 누적 인허가 영역 차트 (영업중 vs 폐업)."""
...
def convex_hull(points: list[tuple[float, float]]) -> list[tuple[float, float]]:
    """Andrew's monotone chain — 점 집합의 볼록 껍질(시계 정렬) 반환."""
...
def dong_focus_map(df: pd.DataFrame, gu: str, clusters: list[str]) -> go.Figure:
    """자치구 핫스팟 동 포커스 지도 — 군집/동 별 강조 폴리곤 + 시설 점."""
    scoped = (
        df[(df["구"] == gu) & (df["cluster_label"].isin(clusters))]
        .dropna(subset=["위도", "경도"])
        .copy()
...
def polygon_line_coords(geometry: dict) -> tuple[list[float], list[float]]:
    """Polygon/MultiPolygon 외곽선 좌표 → (lat list, lon list) 분리 반환."""
    xs: list[float] = []
    ys: list[float] = []
    if not geometry:
        return xs, ys
...
def hex_to_rgba(hex_color: str, alpha: float) -> str:
    """HEX 색상(#RRGGBB) → CSS rgba(...) 문자열로 변환."""
...
def sample_diverging_color(
    ...
    """RdBu_r 발산형 컬러스케일에서 (value-zmin)/(zmax-zmin) 위치 색상 → rgba 문자열."""
...
def full_boundary_coords(boundaries: dict) -> tuple[list[float], list[float]]:
    """모든 동 폴리곤 외곽선을 평탄한 (lat, lon) 좌표 리스트로 합쳐 반환."""
    xs: list[float] = []
    ys: list[float] = []
...
def infer_gu_name(props: dict) -> str | None:
    """피처 properties 에서 자치구명 추론 — '구' 컬럼 우선, 없으면 코드로 매핑."""
    gu_name = props.get("구")
    if gu_name:
        return str(gu_name)
    code = str(
...
def build_gu_boundaries(dong_boundaries: dict) -> dict:
    """법정동 경계 dict 를 자치구 단위로 dissolve 한 GeoJSON 생성."""
...
def add_gu_boundary_trace(
    ...
    *,
    ...
    """지도 figure 에 자치구 외곽선 + 자치구명 라벨 trace 추가."""
    xs: list[float] = []
    ys: list[float] = []
    label_rows = []
...
def grid_effect_rows(
    ...
    """격자별로 시설 점들의 평균값 산출 — 격자 폴리곤에 점-in-polygon 후 평균."""
    points = [
...
def mgwr_grid_seoul_map(
    ...
    """서울 전역 격자 위에 MGWR 변수별 영향력(기여도/계수) 지도 + BW 주석."""
...
def gwr_variable_seoul_map(
    ...
    """서울 전역 GWR 변수별 영향력 점 분포 지도 — 동 경계 + 점 색상으로 표현."""
    coef_col = f"coef_{variable}"
    contribution_col = f"contrib_{variable}"
    z_col = (
        contribution_col
...
def gwr_mgwr_map(gwr_df: pd.DataFrame, gu: str, dong_boundaries: dict) -> go.Figure:
    """자치구 한정 GWR/MGWR local R² 지도 — 동 경계 + 행 단위 점 색상."""
...
def cluster_top_areas(df: pd.DataFrame) -> pd.DataFrame:
    """군집(저/중/고) 별 동 단위 시설 수 TOP — 핫스팟 표 표시용."""
    top = (
...
def risk_factor_chart(df: pd.DataFrame, gu: str) -> go.Figure:
    """자치구 시설들의 위험 요인별 변수 평균 비교 막대 차트."""
    cols = [
        "소방위험도_점수",
        "단속위험도",
        "구조노후도",
...
def point_segment_distance_m(
    ...
    """점(lat,lon) 에서 선분 (a,b) 까지 최단 거리(m) — cosine 보정 평면 근사."""
...
def nearest_point_on_segment(
    ...
    """점(lat,lon) 에서 선분 (a,b) 위 최근접점 위경도 좌표 반환."""
    mean_lat = math.radians((lat + a[0] + b[0]) / 3)
...
def point_distance_m(a: tuple[float, float], b: tuple[float, float]) -> float:
    """두 위경도 점 사이 거리(m) — cosine 보정 평면 근사."""
...
def build_road_graph(
    ...
    """OSM 도로 라인 → 인접 그래프 + 노드 리스트 (다익스트라 최단경로 입력)."""
...
def nearest_graph_node(
    ...
    """그래프 노드 중 point 와 가장 가까운 노드 — 다익스트라 시작점/도착점 진입."""
    if not nodes:
        return None
...
def nearest_fire_center(row: pd.Series, fire_df: pd.DataFrame) -> pd.Series | None:
    """시설 행에서 최근접/담당 안전센터명 → 소방시설 DF 에서 매칭 행 반환."""
    center_names = [
...
def shortest_graph_path(
    ...
    """다익스트라 — 도로 그래프에서 시작점 → 도착점 최단 경로 좌표 + 거리(m)."""
...
def adjacent_road_anchor(row: pd.Series) -> tuple[float, float] | None:
    """시설 row 에 부착된 인접 도로 좌표(있으면) 반환 — 출동경로 시작 anchor."""
...
def shortest_road_path(
    ...
    """시설 → 안전센터 최단 도로 경로 — 인접 도로 앵커 보정 + 다익스트라."""
...
def dispatch_arrival_path(
    ...
    """출동경로(소방→시설) 산출 — shortest_road_path 결과를 역방향으로."""
...
def nearest_road_width(row: pd.Series, road_line_df: pd.DataFrame) -> dict:
    """시설에서 가장 가까운 도로(라인) 의 도로폭 카테고리/거리 정보 반환."""
...
def enrich_representative_roads(
    ...
    """대표 시설 DF 에 nearest_road_width 결과(도로폭/도로명/거리) 컬럼 부착."""
...
def representative_route_facilities(
    ...
    """선택 자치구+군집의 대표 시설 후보 — 위험도 상위 + 도로/경로 메타 결합."""
    route_cols = [
        "구",
        "동",
        "업소명",
...
def route_seconds_label(seconds: float | int | None) -> str:
    """초(seconds) → '5분 30초' 같은 한글 라벨 (None/NaN 은 '-')."""
    if pd.isna(seconds):
        return "-"
...
def route_seconds_only_label(seconds: float | int | None) -> str:
    """초(seconds) → '330초' 같은 짧은 라벨 (분 분리 없이)."""
    if pd.isna(seconds):
        return "-"
    return f"{float(seconds):,.0f}초"
...
def value_or_dash(value: object, suffix: str = "", decimals: int = 1) -> str:
    """값을 포맷팅 — None/NaN 이면 '-' 반환 (대시보드 표시용)."""
    if pd.isna(value):
        return "-"
...
def road_width_adjusted_seconds(
    ...
    """도로폭 보정 도착시간 — 거리×0.12s/m×factor + 도착후 작업 60초."""
...
def selected_route_comparison(
    ...
    """선택 시설의 (직선거리 vs 실제 도로 경로) 비교 메트릭 dict 반환."""
...
def route_width_markers(
    ...
    """경로의 1/4, 2/4, 3/4 지점에서 가장 가까운 도로의 도로폭을 라벨로 표시."""
    if len(path) < 2:
        return pd.DataFrame()
...
def dispatch_route_map(
    ...
    """선택 시설 출동경로 지도 — 직선 거리 + 도로 그래프 최단경로 + 도로폭 마커."""
    dong_points = (
        route_df[
