...
주요 역할:
  1. 숙박시설 좌표를 EPSG:5181 → EPSG:5186 으로 변환한다.
  2. 건물 shapefile 의 모든 건물 중심점에 STRtree 공간 인덱스를 생성한다.
  3. 각 숙박시설 반경 50m 안의 건물들을 용도별(주택/상업/숙박/사무/기타)로 집계한다.
  4. 요약(Summary) 과 상세(Details) 두 가지 결과를 CSV 로 저장한다.

입력:
    data/XY.csv                      (숙박시설 좌표 목록)
    gis/AL_D010_11_20260409.shp       (서울시 건물 shapefile)
출력:
    data/XY_GIS_Analysis_Summary.csv  (숙박시설별 주변 건물 요약)
...
# ── 건물 주용도코드(A8) → 카테고리 분류 사전 ────────────────────────
RESIDENTIAL_CODES = {"01000", "02000"}  # 주택류 (단독, 공동)
...
def analyze():
    """숙박시설별 반경 50m 건물 현황을 분석하여 요약·상세 CSV 를 생성한다."""
    print("Loading target points...")
    # XY.csv 에서 숙박시설 좌표 목록 로드
    ...
    for idx, row in xy_df.iterrows():
        # 원본 좌표를 shapefile 과 같은 좌표계(5186)로 변환
        new_x, new_y = transformer.transform(row["X좌표"], row["Y좌표"])
        # Shapely Point 객체로 만들어 거리 계산에 사용
    ...
    # 서울시 건물 shapefile 읽기 (인코딩: cp949 = 한글 Windows 기본)
    sf = shapefile.Reader(SHP_PATH, encoding="cp949")
    buildings = []

    # 모든 건물 레코드를 순회하며 카테고리 분류 + 중심점 계산
    for i in tqdm.tqdm(range(len(sf)), desc="Reading Buildings"):
        rec = sf.record(i)
        usage_code = rec[8]  # A8: 주용도코드 (예: '01000' = 단독주택)
        ...
        # shapefile 의 도형을 Shapely 객체로 변환 후 중심점 계산
        sh = sf.shape(i)
        ...
        buildings.append(
            {
                ...
                "지상층수": above_floor,
                "지하층수": below_floor,
                "건물ID": rec[1],  # A1: 건물 고유번호
            }
        )

    print("Building spatial index...")
    # 모든 건물 중심점으로 R-Tree 공간 인덱스 생성
    # STRtree 를 쓰면 '반경 50m 내 건물 찾기' 를 전수 검색 대신 빠르게 처리할 수 있다
    ...
    # 각 숙박시설 좌표마다 반경 50m 분석 수행
    for tp in tqdm.tqdm(target_points, desc="Analyzing Points"):
        p = tp["point"]
        # p.buffer(RADIUS) = 반경 50m 원 → tree.query() 로 그 원과 겹치는 건물 후보 인덱스 반환
        indices = tree.query(p.buffer(RADIUS))

        # 카테고리별 카운트 초기화
        counts = {"total": 0, "주택": 0, "상업": 0, "숙박": 0, "사무": 0, "기타": 0}

        # 후보 건물 중 실제 거리 50m 이내인 것만 카운트하고 상세 결과에 기록
        found_buildings = []
        for idx in indices:
        ...
        summary_results.append(
            {
                ...
                "사무_수": counts["사무"],
                "기타_수": counts["기타"],
                "밀집도": f"{counts['total']}개",  # 단순 밀도 표현(개수)
                "주요_시설군": dominant,
                "집중도(%)": concentration,
            }
        )
    ...
    # 요약 / 상세 두 결과를 CSV 로 저장 (한글 안전을 위해 utf-8-sig)
