...
def load_counts() -> pd.DataFrame:
    ...
    df["구"] = df["지번주소"].fillna(df["도로명주소"]).str.extract(r"서울특별시\s+(\S+구)")
    filtered = df[df["년도"].isin(YEARS) & df["구"].isin(GU_10)].copy()

    counts = (
        filtered.groupby(["년도", "구"])
        .size()
        .rename("갯수")
        .reset_index()
    )
...
def plot(counts: pd.DataFrame) -> None:
    ...
    pivot = counts.pivot(index="년도", columns="구", values="갯수").reindex(index=YEARS, columns=gu_order)
    ...
    ax.set_title("외국인관광도시민박업 신규 인허가 건수 (2021-2025, 10개구)", fontsize=18, fontweight="bold", pad=18)
    ax.set_xlabel("년도", fontsize=12, fontweight="bold")
    ax.set_ylabel("신규 인허가 건수", fontsize=12, fontweight="bold")
    ax.set_xticks(x)
    ax.set_xticklabels(YEARS, fontsize=11)
    ax.grid(axis="y", alpha=0.25, linestyle="--")
    ax.legend(title="구", ncol=5, loc="upper left", bbox_to_anchor=(0, 1.03), frameon=True, fontsize=18, title_fontsize=20)
