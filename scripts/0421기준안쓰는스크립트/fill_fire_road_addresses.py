...
"""
[파일 설명]
소방청 특정소방대상물소방시설정보서비스 CSV에서 도로명주소가 없는 행을
카카오 API, 등기부등본 병합 결과, 소방청 정보서비스 원본에서 채우는 스크립트.

주요 역할:
  1. 등기부등본 병합 결과에서 도로명주소 가져오기 (건물일련번호 기준)
  2. 소방청 정보서비스 원본에서 기본주소 가져오기 (대상물명+구명 퍼지 매칭)
  3. 카카오 주소검색 API로 나머지 도로명주소 채우기
  4. 덮어쓰기 저장

입력: data/소방청_특정소방대상물소방시설정보서비스.csv
      data/등기부등본_소방청병합_결과.csv
      data/소방청_특정소방대상물정보서비스.csv
출력: data/소방청_특정소방대상물소방시설정보서비스.csv (덮어쓰기)
"""
import os
import re
import time
...
# 컬럼명 상수 (파일 인코딩 문제 방지용 유니코드 이스케이프)
