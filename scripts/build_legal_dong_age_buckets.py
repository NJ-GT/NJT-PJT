...
SOURCE_PATH = DATA_DIR / "통합숙박시설표제부0415_피처선정본_소방청결합.csv"
KIKMIX_PATH = DATA_DIR / "jscode20200515" / "jscode20200515" / "KIKmix.20200515.xlsx"
OUT_PATH = DATA_DIR / "자치구_법정동별_사용승인연한구간_0415.csv"
...
def load_legal_dong_mapping(path: Path) -> dict[str, dict[str, str]]:
    ...
    with zipfile.ZipFile(path) as zf:
        ...
        for idx, row in enumerate(rows):
            ...
            gu_name = str(record.get("시군구명", "")).strip()
            legal_dong_name = str(record.get("동리명", "")).strip()
            if legal_code:
                mapping[legal_code] = {
                    "구": gu_name,
    ...
    with SOURCE_PATH.open("r", encoding="utf-8-sig", newline="") as fp:
        ...
        for row in reader:
            ...
            if mapped is None:
                ...
                gu_name = mapped["구"]
                legal_dong_name = mapped["법정동명"]

            if not gu_name or not legal_dong_name:
                continue

            key = (gu_name, full_legal_code, legal_dong_name)
            grouped[key]["전체건물수"] += 1
