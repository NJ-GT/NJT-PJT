...
"""
[파일 설명]
공공데이터포털에서 받은 숙박업 response CSV 2개를 등기부등본 표제부와 건물명으로 매핑하는 스크립트.

주요 역할:
  - response #1: 건물명(objNm) 정규화 후 단순 inner join으로 매핑
  - response #2: 1단계(구+동+건물명) → 2단계(구+건물명)로 단계적 매핑 (동 불일치 커버)
  - 매핑 결과를 CSV로 저장하여 후속 분석에 활용한다.

입력: response_1775722530131.csv   (숙박업 인허가 데이터 #1)
      response_1775722813174.csv   (숙박업 인허가 데이터 #2)
      data/등기부등본_표제부_*.csv  (건물 등기 정보)
출력: data/response_등기부등본_매핑.csv    (#1 매핑 결과)
      data/response2_등기부등본_매핑.csv   (#2 매핑 결과)
"""
import pandas as pd
import re
import os
