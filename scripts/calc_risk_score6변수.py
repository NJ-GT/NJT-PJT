...
vars_risk = ['소방위험도_점수', '노후도_점수', '도로폭_정규화',
...
# ── 4. 임시 컬럼 제거 후 저장 ────────────────────────────────────────
df = df.drop(columns=['밀집도_정규화','집중도_정규화',
                       '도로폭_정규화','불법주정차_정규화'])
...
print('\n상위 위험 5개:')
top5 = df.nlargest(5, '위험점수_AHP')[['구','업소명','위험점수_AHP','소방위험도_점수','노후도_점수','공식도로폭m']]
