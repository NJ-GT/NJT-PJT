...
from rag.query_claude import ask_claude
from rag.query_codex import ask_codex
from rag.rag_core import search as rag_search
...
def rag_offline_response(question: str) -> tuple[str, list]:
    chunks = rag_search(question, top_k=5)
    if not chunks:
        return "이 질문은 현재 대시보드 데이터에서 바로 답할 정보가 없습니다.", []

    top_title = chunks[0].title.replace("#", "").strip()
    return (
        f"`{top_title}`와 관련된 내용입니다.\n\n"
        "질문을 조금 더 구체화하면 바로 해석해드릴 수 있습니다. 예: `마포구 요약`, `최종위험점수_new`, `도로망 추정거리`",
        chunks,
    )
...
def safe_eval_math(expression: str) -> float | int | None:
    def eval_node(node: ast.AST) -> float | int:
        ...
        if isinstance(node, ast.UnaryOp) and type(node.op) in SAFE_MATH_OPS:
            return SAFE_MATH_OPS[type(node.op)](eval_node(node.operand))
        raise ValueError
...
def local_basic_answer(question: str) -> str | None:
    normalized = question.strip()
    compact = normalized.replace(" ", "")
    math_match = re.fullmatch(r"([0-9+\-*/().%]+)(은|는|=|\?)?", compact)
    if math_match and any(op in math_match.group(1) for op in "+-*/%"):
        result = safe_eval_math(math_match.group(1))
        if result is not None:
            if isinstance(result, float) and result.is_integer():
                result = int(result)
            return f"{math_match.group(1)} = {result}입니다."

    if compact in {"안녕", "안녕하세요", "ㅎㅇ", "하이"}:
        return "안녕하세요. 대시보드 해석, 변수 의미, 도로망/도로폭, 공간모형 기준을 질문해 주세요."

    if compact in {"뭐할수있어", "뭐할수있어?", "무엇을할수있어", "도움말"}:
        return (
            "0430 기반 대시보드에 대해 답할 수 있습니다. 예를 들면 최종위험점수_new 의미, "
            "도로망 추정거리, 도로폭 보정 예상도착초, 고위험군 기준, 공간모형 결과를 설명할 수 있습니다."
        )

    if "팀" in compact and "이름" in compact:
        return "현재 대시보드/RAG 문서에는 팀 이름 정보가 없습니다."

    cluster_word = "군집" in compact or "클러스터" in compact
    area_word = "같은동네" in compact or "동마다" in compact or "동별" in compact or "같은동" in compact
    if area_word and cluster_word:
        return (
            "군집은 동 단위가 아니라 숙소 단위로 붙습니다.\n\n"
            "같은 동네여도 숙소마다 최종위험점수, 소방 접근성, 도로폭, 단속위험도, 구조노후도, 밀집도 같은 "
            "위험요인 조합이 달라서 고위험군/중위험군/저위험군이 다르게 나올 수 있습니다."
        )

    if cluster_word and ("왜" in compact or "다른" in compact):
        return (
            "`cluster_label`은 숙소별 위험 특성 조합입니다.\n\n"
            "같은 지역이라도 시설별 점수, 소방 접근성, 도로폭, 주변 밀집도가 다르면 군집도 달라집니다."
        )

    return None


def local_dashboard_answer(question: str, selected_gu: str) -> str | None:
    if "final_df" not in globals():
        return None

    compact = question.strip().replace(" ", "")
    gus = sorted(final_df["구"].dropna().unique())
    target_gu = next((gu for gu in gus if gu in compact), None)
    if target_gu is None and compact in {"선택구", "현재구", "이구"}:
        target_gu = selected_gu
    if target_gu is None:
        return None

    summary_words = {"요약", "설명", "어때", "어떰", "분석", "알려줘", "뭐야", "?"}
    is_gu_only = compact in {target_gu, f"{target_gu}?", f"{target_gu}요약", f"{target_gu}분석"}
    if not is_gu_only and not any(word in compact for word in summary_words):
        return None

    gu_rows = final_df[final_df["구"] == target_gu].copy()
    if gu_rows.empty:
        return None

    total = len(gu_rows)
    high = int((gu_rows["cluster_label"] == "고위험군").sum())
    avg_risk = gu_rows["최종위험점수_new"].mean()
    top_dong = gu_rows.groupby("동")["최종위험점수_new"].mean().sort_values(ascending=False).index[0]
    new_2025 = int((gu_rows["승인연도"] == 2025).sum()) if "승인연도" in gu_rows else 0
    factor_table = risk_factor_table(final_df, target_gu)
    top_factors = factor_table.dropna(subset=["전체평균대비"]).head(3)
    factor_text = ", ".join(
        f"{row['위험요인']} {row['전체평균대비']:.1f}"
        for _, row in top_factors.iterrows()
    )

    return (
        f"{target_gu}는 숙박시설 {total:,}개 중 고위험군이 {high:,}개입니다.\n\n"
        f"평균 위험점수는 {avg_risk:.1f}점이고, 평균 위험도가 가장 높은 동은 `{top_dong}`입니다. "
        f"2025년 신규 인허가는 {new_2025:,}개입니다.\n\n"
        f"전체 평균 대비 높은 위험요인은 {factor_text}입니다."
    )


def short_api_error(mode: str, exc: Exception) -> str:
    message = str(exc)
    if "429" in message or "quota" in message.lower() or "credit" in message.lower():
        return f"{mode} API가 현재 응답하지 않아"
    if "API_KEY" in message:
        return f"{mode} API 키가 설정되지 않아"
    return f"{mode} API 호출에 실패해"


def answer_rag_question(selected_gu: str, question: str, mode: str) -> tuple[str, list]:
    basic_answer = local_basic_answer(question)
    if basic_answer:
        return basic_answer, []

    dashboard_answer = local_dashboard_answer(question, selected_gu)
    if dashboard_answer:
        return dashboard_answer, []

    enriched_question = f"선택 구: {selected_gu}\n질문: {question}"
    try:
        if mode == "OpenAI":
            return ask_codex(enriched_question, top_k=5), rag_search(question, top_k=3)
        if mode == "Claude":
            return ask_claude(enriched_question, top_k=5), rag_search(question, top_k=3)
        return rag_offline_response(question)
    except Exception as exc:
        answer, chunks = rag_offline_response(question)
        return answer, chunks


def answer_sidebar_question(selected_gu: str, question: str) -> tuple[str, list]:
    basic_answer = local_basic_answer(question)
    if basic_answer:
        return basic_answer, []
...
def rag_chatbot_panel(selected_gu: str) -> None:
    st.markdown('<div class="section-title">RAG 분석 챗봇</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="soft-note">대시보드 문서, 변수 정의, 공간모형, 도로망/도로폭 설명을 검색해 답변합니다. OpenAI 키만 있으면 OpenAI 모드만 사용하면 됩니다.</div>',
        unsafe_allow_html=True,
    )

    mode = st.selectbox("응답 방식", ["검색만", "OpenAI", "Claude"], index=0)
    st.caption("OpenAI는 `.env`의 OPENAI_API_KEY, Claude는 ANTHROPIC_API_KEY를 사용합니다.")

    if "rag_messages" not in st.session_state:
        st.session_state.rag_messages = [
            {
                "role": "assistant",
                "content": "대시보드 해석, 변수 의미, 공간모형, 도로망/도로폭 기준을 질문해 주세요.",
            }
        ]
    ...
    question = st.chat_input("예: 최종위험점수_new는 뭐야? / 도로망 추정거리는 어떻게 계산돼?")
    if not question:
        return
    ...
    with st.chat_message("assistant"):
        with st.spinner("RAG 문서를 검색하고 답변을 생성하는 중입니다..."):
            answer, chunks = answer_rag_question(selected_gu, question, mode)

        st.markdown(answer)
        if chunks:
            with st.expander("검색 근거 보기"):
                for chunk in chunks:
                    st.markdown(f"**{chunk.source} / {chunk.title}**")
                    st.write(chunk.text)

    st.session_state.rag_messages.append({"role": "assistant", "content": answer})


def rag_sidebar_panel(selected_gu: str) -> None:
    sidebar_chat_version = "local-only-v2"
    if st.session_state.get("rag_sidebar_version") != sidebar_chat_version:
        st.session_state.rag_sidebar_history = []
        st.session_state.rag_sidebar_version = sidebar_chat_version
    if "rag_sidebar_history" not in st.session_state:
        st.session_state.rag_sidebar_history = []

    with st.expander("RAG 챗봇", expanded=True):
        question = st.text_area(
            "질문",
            placeholder="예: 최종위험점수_new는 뭐야?\n도로망 추정거리는 어떻게 계산돼?",
            height=88,
            key="rag_sidebar_question",
        )
        left, right = st.columns([0.62, 0.38])
        ask = left.button("질문하기", use_container_width=True, key="rag_sidebar_ask")
        clear = right.button("지우기", use_container_width=True, key="rag_sidebar_clear")
        ...
        if ask and question.strip():
            with st.spinner("RAG 검색 중..."):
                answer, chunks = answer_sidebar_question(selected_gu, question.strip())
            st.session_state.rag_sidebar_history.insert(
                0,
                {
            ...
            st.session_state.rag_sidebar_history = st.session_state.rag_sidebar_history[:3]

        if not st.session_state.rag_sidebar_history:
            st.caption("대시보드 변수, 도로망, 공간모형, 위험점수 기준을 바로 물어볼 수 있습니다.")
            return

        latest = st.session_state.rag_sidebar_history[0]
