# -*- coding: utf-8 -*-
"""
집계구별 화재위험도 박스플롯 분석 + ECharts HTML 생성 스크립트.

목적:
    숙박시설이 있는 집계구만 필터링하여 자치구별 화재위험도 분포를
    박스플롯(Min/Q1/Median/Q3/Max + 이상치)으로 시각화한다.
    아래쪽엔 정렬/필터/검색이 가능한 상세 테이블이 붙는다.

화재위험도 계산식 (외부 데이터에서 이미 계산되어 있음):
    fire_score = 노후도 30% + 건폐율 25% + 용적률 25% + 층수 20%

데이터:
    입력 : data/oa_density.json (집계구별 fire_score, count 등)
    출력 : 위험도_분석.html
"""
...
# 10개 구 + 숙박시설이 1개라도 있는 집계구만 추출 (count > 0)
rows = []
for f in d["features"]:
    p = f["properties"]
...
# 정렬: GU 표시 순서 우선, 같은 구 내에서는 화재위험도 높은 순
...
def bp_stats(scores):
    """
    박스플롯에 필요한 5수치([lo, Q1, Q2, Q3, hi])와 이상치 리스트 반환.

    - IQR = Q3 - Q1
    - whisker: Q1 - 1.5*IQR ~ Q3 + 1.5*IQR
    - 이 범위를 벗어난 값은 이상치로 분리
    - 단, whisker는 실제 데이터 max/min 을 넘지 않도록 클립
    """
    arr = np.array(scores, dtype=float)
    q1, q2, q3 = np.percentile(arr, [25, 50, 75])
    iqr = q3 - q1
    # 하한 — 실제 최솟값보다 낮으면 실제 최솟값 사용
    lo = max(float(arr.min()), q1 - 1.5 * iqr)
    # 상한 — 실제 최댓값보다 높으면 실제 최댓값 사용
...
for gu in GU:
    ...
    if scores:
        ...
        # X축 인덱스(현재 추가된 라벨의 위치)
        idx = len(bp_labels) - 1
        for o in outs:
            # 이상치는 [x인덱스, y값] 좌표로 산점도에 함께 표시
