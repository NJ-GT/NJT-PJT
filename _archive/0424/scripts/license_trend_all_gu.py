...
for GU in GU_10:
    ...
    민박_영업 = (f1[f1['인허가연도'].between(2020,2024) & (f1['영업상태명']=='영업/정상')]
                 .groupby('인허가연도').size().reindex(YEARS, fill_value=0))

    동_pivot_raw = (f1[f1['인허가연도'].between(2020,2024)]
    ...
    숙박_yr = (f3[f3['인허가연도'].between(2020,2024)]
               .groupby(['인허가연도','업태']).size()
               .unstack(fill_value=0)
               .reindex(index=YEARS, fill_value=0))
    관광_yr = (f2[f2['인허가연도'].between(2020,2024)]
               .groupby(['인허가연도','업태']).size()
               .unstack(fill_value=0)
               .reindex(index=YEARS, fill_value=0))

    fig = plt.figure(figsize=(18, 12), facecolor=BG)
    fig.suptitle(f'{GU} 숙박업 신규 인허가 트렌드 (2020–2024)',
    ...
    summary = {
        '외국인관광\n도시민박업': (int(민박_yr.get(2020, 0)), int(민박_yr.get(2024, 0))),
