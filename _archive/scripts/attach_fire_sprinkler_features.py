...
"""
[파일 설명]
통합숙박시설 피처 CSV와 소방청 특정소방대상물 주소피처 CSV를 이름+주소 기준으로 매핑하여
스프링클러 설치 여부를 숙박시설 피처에 결합하는 스크립트.

주요 역할:
  1. 사업장명과 주소를 정규화하여 역인덱스(이름 인덱스, 주소 인덱스) 생성
  2. 이름+주소 동시 일치 → 단일 후보일 때 매핑 확정
  3. 불일치/복수후보/이름만/주소만 일치 건 각각 별도 CSV로 저장

입력: data/통합숙박시설표제부0415_피처선정본.csv
      data/소방청_특정소방대상물_주소피처.csv
출력: data/통합숙박시설표제부0415_피처선정본_소방청결합.csv (메인 결과)
      data/통합숙박시설표제부0415_피처선정본_소방청_이름만일치.csv
      data/통합숙박시설표제부0415_피처선정본_소방청_주소만일치.csv
      data/통합숙박시설표제부0415_피처선정본_소방청_이름주소각자일치.csv
      data/통합숙박시설표제부0415_피처선정본_소방청_복수후보.csv
"""
import csv
import re
import unicodedata
...
def clean_text(value):
    """CSV에서 읽은 값의 앞뒤 공백, 이중 따옴표, 연속 공백을 정리한다."""
    if pd.isna(value):
        return ""
...
def normalize_text(value):
    """NFKC 유니코드 정규화 후 공백 정리 (전각 문자 → 반각 등)."""
    return unicodedata.normalize("NFKC", clean_text(value)).strip()


def normalize_address(value):
    """주소 비교용 키 생성: 소문자화 + '번지' 제거 + 괄호 제거 + 영숫자·한글만 남김."""
...
def name_variants(text):
    """사업장명의 정규화 변형 집합을 반환한다 (괄호/슬래시/앰퍼샌드 분리 포함)."""
...
def read_fire_csv(path):
    """소방청 주소피처 CSV를 읽는다. 컬럼 수 불일치 행은 마지막 컬럼을 합쳐서 처리."""
...
def candidate_preview(fire, indices):
    """매핑 후보 상위 5개를 '대상물명 / 지번 / 도로명' 형식으로 미리보기 문자열 반환."""
    previews = []
...
def build_fire_indexes(fire):
    """소방청 데이터를 이름 역인덱스(name_index)와 주소 역인덱스(addr_index)로 변환한다.

    역인덱스: key(이름 변형 또는 주소 키) → {소방청 행 인덱스 집합}
    """
    fire = fire.copy()
